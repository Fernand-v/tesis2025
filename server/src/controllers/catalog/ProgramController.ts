...
import ProgramService from '../../services/catalog/ProgramService';
...
const normalizeBoolean = (value: unknown, defaultValue = 0): number => {
...
const ProgramController = {
  list: async (_req: Request, res: Response) => {
    const overview = await ProgramService.overview();
    res.json(overview);
  },
  ...
  update: async (req: Request, res: Response) => {
    ...
    const programa = await ProgramService.update(codigo, {
      descripcion,
      ubicacion,
      formulario,
      habilitado,
  ...
  remove: async (req: Request, res: Response) => {
    ...
    const removed = await ProgramService.remove(codigo);
