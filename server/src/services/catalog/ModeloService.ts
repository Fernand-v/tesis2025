...
export default class ModeloService {
  static async list(): Promise<Modelo[]> {
    const pool = getPool();
  ...
  static async create(descripcion: string): Promise<Modelo> {
    const pool = getPool();
  ...
  static async update(codigo: number, descripcion: string): Promise<Modelo | null> {
    const pool = getPool();
