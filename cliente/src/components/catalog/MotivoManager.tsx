...
const MotivoManager = () => {
  ...
  const fetchMotivos = useCallback(async () => {
    setLoading(true);
    try {
      const response = await authFetch('/catalog/motives');
      const data = (await response.json()) as { motivos?: Motivo[] };
  ...
  useEffect(() => {
    setPageTitle('Motivos');
    void fetchMotivos();
    return () => setPageTitle('Sistema General');
  }, [fetchMotivos, setPageTitle]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingCodigo === null) {
        await authFetch('/catalog/motives', {
          method: 'POST',
          body: JSON.stringify({ descripcion: value }),
        });
        showFeedback('Motivo creado.');
      } else {
        const response = await authFetch(`/catalog/motives/${editingCodigo}`, {
          method: 'PUT',
          body: JSON.stringify({ descripcion: value }),
        });
        if (response.status === 404) {
          showFeedback('Motivo no encontrado.', 'error');
        } else {
          showFeedback('Motivo actualizado.');
        }
      }
  ...
  const handleDelete = useCallback(async () => {
    ...
    clearFeedback();
    try {
      const response = await authFetch(`/catalog/motives/${confirmDelete}`, { method: 'DELETE' });
      if (response.status === 404) {
        showFeedback('Motivo no encontrado.', 'error');
      } else {
        showFeedback('Motivo eliminado.');
  ...
  }, [authFetch, clearFeedback, confirmDelete, editingCodigo, fetchMotivos, resetForm, showFeedback]);
    ...
    <div className={styles.panel}>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className={`text-lg font-semibold ${styles.heading}`}>Motivos</h2>
            <p className={`text-xs ${styles.subheading}`}>Catalogo de motivos utilizado en inventarios y recepciones.</p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <section className={`${styles.section} overflow-hidden`}>
            ...
            <div className={`divide-y ${styles.divider}`}>
              ...
              {motivos.map((motivo) => (
                ...
                >
                  <span className="text-xs opacity-70">{motivo.codigo}</span>
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => setConfirmDelete(motivo.codigo)}
          ...
          <form onSubmit={handleSubmit} className={`${styles.section} space-y-4 p-5`}>
            <h3 className={`text-sm font-semibold ${styles.heading}`}>
              {editingCodigo === null ? 'Nuevo motivo' : `Editar motivo #${editingCodigo}`}
            </h3>
      ...
      <ConfirmDialog
        ...
        description="Esta accion no se puede deshacer. El motivo sera eliminado permanentemente."
