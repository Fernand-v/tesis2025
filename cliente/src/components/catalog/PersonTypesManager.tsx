...
const PersonTypesManager = () => {
  ...
  const [tipos, setTipos] = useState<PersonType[]>([]);
  ...
  const fetchTipos = useCallback(async () => {
    setLoading(true);
    try {
      const response = await authFetch('/catalog/person-types');
      const data = (await response.json()) as { tipos?: PersonType[] };
      setTipos(data.tipos ?? []);
    } catch (err) {
  ...
  useEffect(() => {
    setPageTitle('Tipos de persona');
    void fetchTipos();
    return () => setPageTitle('Sistema General');
  }, [fetchTipos, setPageTitle]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingCodigo === null) {
        await authFetch('/catalog/person-types', {
          method: 'POST',
          body: JSON.stringify({ descripcion: value }),
        });
        showFeedback('Tipo de persona creado.');
      } else {
        await authFetch(`/catalog/person-types/${editingCodigo}`, {
          method: 'PUT',
          body: JSON.stringify({ descripcion: value }),
        });
        showFeedback('Tipo de persona actualizado.');
      }
  ...
  const handleDelete = useCallback(async () => {
    ...
    setSaving(true);
    try {
      await authFetch(`/catalog/person-types/${confirmDelete}`, { method: 'DELETE' });
      if (editingCodigo === confirmDelete) {
        resetForm();
      }
      showFeedback('Tipo de persona eliminado.');
      await fetchTipos();
    } catch (err) {
      showFeedback((err as Error).message, 'error');
    } finally {
  ...
  }, [authFetch, confirmDelete, editingCodigo, fetchTipos, resetForm, showFeedback]);
    ...
    <div className={styles.panel}>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className={`text-lg font-semibold ${styles.heading}`}>Tipos de persona</h2>
            <p className={`text-xs ${styles.subheading}`}>
              Clasificaciones asociadas a las personas registradas en el sistema.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <section className={`${styles.section} overflow-hidden`}>
            <div className="grid grid-cols-[80px_1fr_110px] border-b border-white/5 px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">
              <span>Codigo</span>
            ...
            <div className={`divide-y ${styles.divider}`}>
              ...
              {!loading && tipos.length === 0 && (
                <p className={`px-4 py-6 text-center text-xs ${styles.subheading}`}>
                  Aun no se registraron tipos de persona.
                </p>
              )}
              ...
              {tipos.map((tipo) => (
                <div
                  key={tipo.codigo}
                  className={`grid grid-cols-[80px_1fr_110px] items-center gap-2 px-4 py-2 text-sm ${styles.textPrimary} ${styles.hoverLift}`}
                >
                  <span className="text-xs opacity-60">{tipo.codigo}</span>
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => setConfirmDelete(tipo.codigo)}
          ...
          <form onSubmit={handleSubmit} className={`${styles.section} space-y-4 p-5`}>
            <h3 className={`text-sm font-semibold ${styles.heading}`}>
              {editingCodigo === null ? 'Nuevo tipo' : `Editar tipo #${editingCodigo}`}
            </h3>
...
export default PersonTypesManager;
