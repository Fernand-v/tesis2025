...
const ProgramTypesManager = () => {
  ...
  const fetchTipos = useCallback(async () => {
    setLoading(true);
    try {
      const response = await authFetch('/catalog/program-types');
      const data = (await response.json()) as { tipos?: ProgramType[] };
      setTipos(data.tipos ?? []);
    } catch (err) {
  ...
  useEffect(() => {
    setPageTitle('Tipos de programa');
    void fetchTipos();
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingCodigo === null) {
        await authFetch('/catalog/program-types', {
          method: 'POST',
          body: JSON.stringify({ descripcion: value }),
        });
        showFeedback('Tipo de programa creado.');
      } else {
        await authFetch(`/catalog/program-types/${editingCodigo}`, {
          method: 'PUT',
          body: JSON.stringify({ descripcion: value }),
        });
        showFeedback('Tipo de programa actualizado.');
      }
  ...
  const handleDelete = useCallback(async () => {
    ...
    setSaving(true);
    try {
      await authFetch(`/catalog/program-types/${confirmDelete}`, { method: 'DELETE' });
      if (editingCodigo === confirmDelete) {
        resetForm();
      }
      showFeedback('Tipo de programa eliminado.');
      await fetchTipos();
    } catch (err) {
      showFeedback((err as Error).message, 'error');
    } finally {
    ...
    <div className={styles.panel}>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className={`text-lg font-semibold ${styles.heading}`}>Tipos de programa</h2>
            <p className={`text-xs ${styles.subheading}`}>
              Catalogo utilizado para agrupar los programas del sistema.
            </p>
          </div>
          <button
            type="button"
            ...
            <div className={`divide-y ${styles.divider}`}>
              ...
              {!loading && tipos.length === 0 && (
                <p className={`px-4 py-6 text-center text-xs ${styles.subheading}`}>
                  Aun no se registraron tipos de programa.
                </p>
              )}
              {tipos.map((tipo) => (
                <div
...
export default ProgramTypesManager;
