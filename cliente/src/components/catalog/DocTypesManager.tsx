...
const DocTypesManager = () => {
  ...
  const [tipos, setTipos] = useState<DocType[]>([]);
  ...
  useEffect(() => {
    setPageTitle('Tipos de documento');
    void fetchTipos();
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingCodigo === null) {
        await authFetch('/catalog/doc-types', {
          method: 'POST',
          body: JSON.stringify({ descripcion: value }),
        });
        showFeedback('Tipo de documento creado.');
      } else {
        await authFetch(`/catalog/doc-types/${editingCodigo}`, {
          method: 'PUT',
          body: JSON.stringify({ descripcion: value }),
        });
        showFeedback('Tipo de documento actualizado.');
      }
  ...
  const handleDelete = useCallback(async () => {
    ...
    setSaving(true);
    try {
      await authFetch(`/catalog/doc-types/${confirmDelete}`, { method: 'DELETE' });
      if (editingCodigo === confirmDelete) {
        resetForm();
      }
      showFeedback('Tipo de documento eliminado.');
      await fetchTipos();
    } catch (err) {
      showFeedback((err as Error).message, 'error');
    } finally {
    ...
    <div className={styles.panel}>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className={`text-lg font-semibold ${styles.heading}`}>Tipos de documento</h2>
            <p className={`text-xs ${styles.subheading}`}>
              Estandariza los documentos aceptados en el sistema.
            </p>
          </div>
          <button
            type="button"
...
export default DocTypesManager;
